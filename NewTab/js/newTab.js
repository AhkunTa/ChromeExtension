    ...
    function (config) {
      ...
      if (!settings.setRandomBg) {
        ...
        if (config.bgImg !== "") {
          ...
          let radomColor = `rgb(${Math.round(
            Math.random() * 100
          )}, ${Math.round(Math.random() * 100)} , ${Math.round(
            Math.random() * 100
          )})`;
          $("#background").show().css({
            "background-color": radomColor,
            opacity: "1",
          });
        }
      } else {
      ...
      items
        .find("[name=showChickenSoup]")
        .prop("checked", config.showChickenSoup);
...
function updataOneMsg() {
  $(".poetry-whole").css({
    "max-width": `${document.documentElement.clientWidth - 200}px`,
  });
  $.ajax({
    type: "get",
    url: oneUrl,
  ...
  $.ajax({
    ...
    success: function (data) {
      if (url == bingUrl) {
        ...
        let bgUrl =
          "url(https://cn.bing.com" + data.images[randomBgIndex - 1].url + ")";
        $("#background").css({
          "background-image": bgUrl,
        });
        backgroundLoaded($("#background"), true, function () {
          if (
            $(".select-radom .radio-item [type=radio]:checked").attr("value") ==
            "false"
          ) {
            $("#background-image").attr({
              src: settings.setBgImg,
            });
            $("#background-image").fadeIn();
            $(".select-background").show();
            $(".select-background-url").hide();
    ...
    error: function (data) {
      let radomColor = `rgb(${Math.round(Math.random() * 100)}, ${Math.round(
        Math.random() * 100
  ...
  switch (checkName) {
    ...
    case "showChickenSoup":
      ...
      if (value) {
        $(".select-chicken-soup").show();
        if (
          $(".select-chicken-soup").find("input[type=checkbox]").is(":checked")
        ) {
          $(".select-chicken-soup .check-title").text("来碗鸡汤");
